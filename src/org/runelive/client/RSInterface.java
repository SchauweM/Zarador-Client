...
public class RSInterface {
	...
	public static void addText(int i, int orientation, String s, int k, boolean centered, boolean m, int a, TextDrawingArea[] TDA,
			int j) {
		RSInterface rsinterface = addInterface(i);
		rsinterface.parentID = i;
		rsinterface.id = i;
	...
	static void unpack(Archive streamLoader, TextDrawingArea textDrawingAreas[], Archive streamLoader_1, RSFontSystem[] newFontSystem) {
		...
		while (stream.position < stream.buffer.length) {
			...
			if (rsInterface.type == 4) {
				rsInterface.message = stream.getString().replaceAll("RuneScape", "Zarador");
	...
	public RSFontSystem rsFontSystem;
	RSFontSystem rsFont;
	boolean textShadow;
